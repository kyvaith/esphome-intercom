...
### YAML: legacy full-experience media player speaker fork

This was the `2026.6.2` compatibility path for full-experience YAMLs that still
used ESPHome's `platform: speaker` media player. Current maintained dev YAMLs
have moved to the source-based `speaker_source` media player instead; this
section remains for custom YAMLs copied from the older `platform: speaker`
layout.

Custom full-experience YAMLs that still use `platform: speaker` media playback
must either migrate to `packages/media_player/full_mono_48k.yaml` or carry both
pieces:
...
Native intercom-only YAMLs and current source-based full profiles do not need
this fork.
